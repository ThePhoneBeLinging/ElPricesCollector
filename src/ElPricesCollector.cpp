...
std::shared_ptr<HourPrice> ElPricesCollector::getLastPrice() const
{
  auto currentTime = std::chrono::system_clock::now();
  currentTime -= std::chrono::hours(1);
  auto timeTM = TimeUtil::timeToTM(currentTime);
...
void ElPricesCollector::update()
{
  ...
  cpr::Response r = cpr::Get(cpr::Url{
    "https://andelenergi.dk/?obexport_format=csv&obexport_start=" + firstDayInWeekString + "&obexport_end=" +
    lastDayInWeekString + "&obexport_region=east&obexport_tax=0&obexport_product_id=1%231%23TIMEENERGI"
  });
